"""
Model DenseNet : params: 177053
Model DenseNet : size: 0.675404M
"""
...
class DenseNet(nn.Module):
    def __init__(self, growthRate, depth, reduction, nClasses, bottleneck):
        ...
        :param growthRate: DenseNet 卷积核增长率，控制卷积核的数量
        :param depth: DenseNet 层数
        :param reduction:  论文中的通道减少率，
        :param nClasses:    输出数量
        :param bottleneck:   是否采用瓶颈结构
        """
        super(DenseNet,self).__init__()
        # 堆叠的block数量    其中  4表示 第1个卷积层，2个transition，和1个FC层， 3表示3个block
        nDenseBlocks = (depth-4)//3

        if bottleneck:
            nDenseBlocks //= 2  # 基础模块中有2层
    ...
    def _make_denseblock(self, nChannels, growRate, nDenseBlocks, bottleneck):
        """
        创建denseblock
        :param nChannels: 进入block的特征图通道数
        :param growRate:  int
        :param nDenseBlocks: 堆叠数量
        :param bottleneck: 是否需要bottleneck
        :return:
        """
        ...
        for i in range(nDenseBlocks):
            if bottleneck:
                ...
                layers.append(SingleLayer(nChannels, growRate))
            nChannels += growRate
    ...
    def forward(self, x):
        ...
        # 3/3 池化+全连接，输出分类
        out = torch.squeeze(self.avgpool(out))
...
if __name__=="__main__":
    model = DenseNet(growthRate=12, depth=40, reduction=0.5, bottleneck=True, nClasses=17)
